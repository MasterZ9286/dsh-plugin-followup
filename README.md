...
| 不打断阅读位置 | 发送追问时**钉住主对话的滚动位置**：你在上面读哪一段就停在哪一段，不会被"深度求索中"的状态行拽到底部；自己滚动或按键即解除 |
...
- **阅读位置守卫**：发送前记录会话滚动容器的 `scrollTop`，随后以 200ms 间隔校正（最长 120 秒）；`wheel` / `touchstart` / 翻页按键 / 在对话区内按下指针都会立即解除守卫，把滚动控制权交还用户。发送时若本来就贴在底部，则不介入。

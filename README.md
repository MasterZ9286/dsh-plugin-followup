...
| 与主对话同款渲染 | 面板直接复用产品自己的 `MarkdownText`（来自 `@deepseek-ai/dsh-client-ui-primitives`），代码块、表格、列表、公式的呈现与主对话完全一致，不会露出原始的 ``` 围栏 |
...
- **同款 Markdown**：通过 `dsh.client.external` 声明后 `require('@deepseek-ai/dsh-client-ui-primitives')` 拿到 `MarkdownText` —— 就是主对话渲染助手正文的那个组件，所以面板里的排版天然一致；拿不到时退回纯文本。
